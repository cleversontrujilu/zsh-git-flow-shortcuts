...
> Repository: `cleversontrujilu/zsh-git-flow-shortcuts` · plugin name: `git-flow-shortcuts`.
...
Clone the repository into a folder named `git-flow-shortcuts` (it's the folder
name, not the repo name, that oh-my-zsh uses to find the plugin):
...
# antidote (.zsh_plugins.txt file)
cleversontrujilu/zsh-git-flow-shortcuts
```

## Bonus: skill for Claude Code

In [`bonus/skill-claude-code/`](bonus/skill-claude-code/) there is a skill that lets
**Claude Code** drive this same workflow (open/close feature, release and hotfix),
choosing branch names, SemVer versions and writing commits and changelog from a
real analysis of the diff. It orchestrates this plugin's functions — it does not
reimplement git flow.
